...
d3.select('div#task1a')			// Selects the <div> HTML element with id="task1a"
	...
	.style('color', 'red');		// Sets the style of the text, in this case the color of the text will be red
...
d3.select('div#task2b')
	...
	.enter()
	.append('p')
	.text(function (d, i) {			// Instead of setting the text to a fixed string we can instead pass a function
		return i + ' ' + d * 5;		// d is the particular data element within the Array, i is the index of that element
...
var xScale = d3.scaleLinear()                       // create a linear scale
	...
	.range([padding, width - padding]);				// Range is the min/max of the SVG you want your data mapped to
...
var color = d3.scaleOrdinal(d3.schemeCategory20);	// Ordinal Scales are categories
                                                    // schemeCategory20 is a predefined category of 20 colours
...
var objArray = [{ // Create an array of objects with x and y co-ordinates, this format is also referred to as JSON
	x: 1,
	y: 4
}, {
	x: 2,
...
var xmax = d3.max(objArray, function (d) {	// xmax will return the object with the max x value from the objArray
	return d.x;
});

var ymax = d3.max(objArray, function (d) {	// ymax will return the object with the max y value from the objArray
	return d.y;
});
...
// This allows us to create multiple things for each data point, in this case a <circle> and a <text> for every point in our Array

var groups = d3.select('svg#task6')
	.selectAll('g.datapoint')        // Select all svg 'groups', refered to as <g>, which have the 'class' of datapoint
	...
	.attr('transform', 'translate(' + xScale(0) + ',' + yScale(0) + ')');  // <g> position is defined using a transform

groups.transition().duration(2000)   // after two second move the group
...
groups.append('circle')               // for every group that has been added, append a circle within that group
	...
	.transition().duration(2000)
	.style('fill', function (d, i) {
		return color(i);              // set the color of each circle to be a color from the 'color' scale
	});
...
groups.append('text')                           // add a piece of text for each group, which will act as our label
	...
	.attr('transform', 'translate(-10, -5)')    // shift the position of the text up and left from where the circle is
	.style('text-anchor', 'end');               // set the 'end' of the text to this position (right-align)
...
d3.select('svg#task7')
	...
	.attr("transform", "translate(0," + (height - padding) + ")")  // move this group to the bottom of the svg
	.call(xAxis);                                                  // create the axis within this group

d3.select('svg#task7')
	.append('g')                                                   // append a group to our <svg>
	.attr("transform", "translate(" + padding + "," + 0 + ")")     // move this group a distance of 'padding' in x-axis
    .call(yAxis);                                                  // create the axis within this group

// Task 8. Interaction

var xAxis = d3.axisBottom(xScale);  // create an axis variable that show labels on the *bottom* and uses xScale domain
var yAxis = d3.axisLeft(yScale);    // create an axis variable that show labels on the *left* and uses yScale domain
